...
import { type Player, type GameMode, type TierLevel, updatePlayerGlobalPoints } from "@/services/playerService"
...
export const useAdminPanel = () => {
  ...
  const refreshPlayers = async () => {
    ...
    try {
      ...
      const { data, error } = await supabase.from("players").select("*").order("global_points", { ascending: false })
  ...
  const findPlayerByIGN = async (ign: string): Promise<{ id: string; ign: string } | null> => {
    try {
      const { data, error } = await supabase.from("players").select("id, ign").ilike("ign", ign).single()
  ...
  const updatePlayerTier = async (playerIdOrIGN: number | string, gamemode: GameMode, tier: TierLevel) => {
    ...
    if (typeof playerIdOrIGN === "string" && isNaN(Number(playerIdOrIGN))) {
      ...
      const numericId = typeof playerIdOrIGN === "string" ? Number.parseInt(playerIdOrIGN) : playerIdOrIGN
    ...
    try {
      ...
      const { error } = await supabase.from("gamemode_scores").upsert(upsertData, { onConflict: "player_id,gamemode" })
  ...
  const deletePlayer = async (playerId: string) => {
    ...
    try {
      ...
      deepSeekService.logDatabaseOperation(
        "DELETE",
        "players",
        { id: playerId },
        {
  ...
  const submitPlayerResults = async (
    ...
    try {
      ...
      const finalJavaUsername = java_username && java_username.trim() ? java_username.trim() : "itzrealme"
      ...
      const duration = Date.now() - startTime
      deepSeekService.logApiCall(
        "POST",
        "/admin/submit-results",
        { ign, resultsCount: results?.length },
        { success: true },
        null,
        duration,
      )
